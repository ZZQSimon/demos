 ...
 * @returns
 */
echartUtils.exceptionChart = function(_) {
	var option = {
		noDataLoadingOption : defaultNoDataLoadingOption,
		series : [ {} ]
	}
	option.setText = function(_) {
...
echartUtils.generateCommonOption = function() {
	...
	option.setdataZoomY = function(_) {
		this.dataZoom.y = _;
	};
...
echartUtils.capacityTreemap = function() {
	...
	option.fillSeries = function() {
		...
		if(valueCA.length>0 && seriesCapacityObject.data.length==0){
  			option.noDataLoadingOption.text = '经查，没有剩余容量';
		}
