...
import com.simon.framework.timer.TaskPeriod;
import com.simon.framework.timer.TaskPriority;
import com.simon.framework.timer.TaskUnit;

/**
 * 
 * @ClassName: TaskUnitImpl <br>
 * @Description: 定时任务配置文件封装对象类 <br>
...
public class TaskUnitImpl implements TaskUnit {
	...
	public TaskUnitImpl(String name, String priority, String period, String delay, Class<?> task, String startTime,
			String runnable) {
		this.name = name;
	...
	private void initStartTime(String startTime) {
		...
		try {
			this.startTime = new Date(format.parse(startTime).getTime() + this.getDelay());

			while (this.getPeriod() > 0 && this.startTime.getTime() < this.currentTime.getTime()) {
				if (this.startTime.getTime() < this.currentTime.getTime()) {
					this.startTime = new Date(this.startTime.getTime() + this.getPeriod());
				}
			}
	...
	public boolean isRunnable() {
		...
		if (this.getPeriod() == 0 && this.startTime.getTime() < this.currentTime.getTime()) {
			return false;
		}
		return true;
	}
